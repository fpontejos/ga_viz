...
def get_som_cds(df, features_orig):
    ...
    som_dfo, som_gp = get_som(df, features_orig)
    ...
    som_df["text"] = hits_labels
    som_df["hits"] = hits_labels
    som_df["hits_pct"] = hits_pct * size * 0.75
    som_df["coords"] = coords
    som_df["radius"] = hits_flat * radius_multiplier
    som_df["node_index"] = node_indices
    som_df["_current_hex"] = som_df["umatrix"]

    for i in range(len(features)):
        som_df["SOM_" + features_orig[i]] = som_gp.get_weights()[:, :, i].reshape(m * n)
        som_df[features_orig[i]] = som_gp.get_weights()[:, :, i].reshape(m * n)

    som_cdsx = ColumnDataSource(som_df)
    print(som_df.columns)
    print(som_dfo.columns)
